...
//        Y = featureNormalize.getFeatureNormalize(Y);

        /***** cost positive and negative start ****/
        /**
         * Y has a row of classes and depending on the value of y[i] particular class of Y is 1
         */
        double[][] Y;
        if(multiclass){
            Y = new double[Xrow][numlabels]; //Y in octave backpropagation
            int count = 0;
...
//                    System.out.println(yRowWiseVal);
                    count++;
                }
            }
        }else{
                ...
                for(int i=0; i<yRow; i++){
                    int yRowWiseVal = (int)y[i][0]; // if it has a class 0 use this
                    Y[i][yRowWiseVal] = 1;
                }
        }
        double[][] transLogH = matrix.transpose(logOfH);
        double[][] transLogOneMinH = matrix.transpose(logOneMinusH);
        double[][] costPositive = new double[Y.length][Y[0].length];
        double[][] costNegative = new double[Y.length][Y[0].length];
        double[][] costArr = new double[Y.length][Y[0].length];
        double sumCost = 0;
        for(int i=0; i<Y.length; i++){
            for(int j=0; j<Y[0].length; j++){
                costPositive[i][j] = -Y[i][j]*transLogH[i][j];
                costNegative[i][j] = (1-Y[i][j])*transLogOneMinH[i][j];
                costArr[i][j] = costPositive[i][j] - costNegative[i][j];
                sumCost += costArr[i][j];
            }
        }
